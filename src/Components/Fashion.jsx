...
const Fashion = () => {
  ...
  const brandOptions = [...new Set(products.filter(p => p.category.includes('fashion')).map(p => p.brand))].sort();
  const colorOptions = [...new Set(products.filter(p => p.category.includes('fashion')).map(p => p.color))].sort();
    ...
    <div>
      ...
      <div className="flex gap-2">
        ...
        <div className="w-[24%] bg-white sticky top-36 h-[calc(100vh-140px)] overflow-y-auto overflow-x-hidden pr-2 custom-scroll z-10">
          <FashionSidebarFilter
            categoryData={fashionCategory}
