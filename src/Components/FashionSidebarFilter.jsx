            ...
            {categoryData.sub.map((subcat, i) => (
              <div key={i}>
                {Array.isArray(subcat.sub) && subcat.sub.length > 0 ? (
                  <>
                    <div className="flex items-center justify-between font-semibold text-gray-800 text-[15px] mb-1 cursor-pointer"
                         onClick={() => setExpandedSubcategory(
                           expandedSubcategory === subcat.name ? null : subcat.name
                         )}>
                      <span>{subcat.name}</span>
                      <span className="text-xs">
                        {expandedSubcategory === subcat.name ? <FaChevronUp /> : <FaChevronDown />}
                      </span>
                    </div>
                    {expandedSubcategory === subcat.name && (
                      <div className="ml-2 pl-2 border-l max-h-[130px] overflow-y-auto visible-scrollbar space-y-1">
                        {subcat.sub.map((item, j) => (
                          <label key={j} className="flex items-center gap-2 text-[14px] text-gray-700 cursor-pointer">
                            <input
                              type="checkbox"
      ...
      <div>
        ...
        >
          {colorOptions.map((color, i) => (
            <label key={i} className="flex items-center gap-2 text-[14px] text-gray-700 cursor-pointer ml-2 mb-1">
              <input
                type="checkbox"
      ...
      <div>
        ...
        <div className="space-y-2 ml-1">
          {[50, 30, 10].map((discount) => (
            <label key={discount} className="flex items-center gap-2 text-[14px] text-gray-700 cursor-pointer">
              <input
                type="checkbox"
...
export default FashionSidebarFilter;
