...
const FurnituresSidebarFilter = ({
  ...
  if (!categoryData) return <p>Loading filters...</p>;

  return (
    <div className="ml-10 space-y-6">
      {/* ✅ Category Filter */}
      {categoryData?.sub?.length > 0 && (
        <div>
          <div
              ...
              {categoryData.sub.map((subcat, i) => (
                <div key={i}>
                  {Array.isArray(subcat.sub) && subcat.sub.length > 0 ? (
                    <>
                      <div
                        className="flex justify-between font-semibold text-gray-800 text-[15px] cursor-pointer"
                        onClick={() =>
              ...
              {["Min", "Max"].map((label, index) => (
                <div key={index} className="flex gap-1 items-center">
                  ...
                  <input
                    type="number"
                    value={
                      priceRange[index] === Infinity ? "" : priceRange[index]
                    }
...
export default FurnituresSidebarFilter;
