...
const Fashion = () => {
  const bagsCategory  = categories.find(cat => cat.name === "Bags");
  ...
  const brandOptions = [...new Set(products.filter(p => p.category.includes('bags')).map(p => p.brand))].sort();
  const colorOptions = [...new Set(products.filter(p => p.category.includes('bags')).map(p => p.color))].sort();
  ...
  // Filter products
  const bagsProducts  = products.filter(p =>
