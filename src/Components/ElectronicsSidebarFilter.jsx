...
const ElectronicsSidebarFilter  = ({
  categoryData,
  onRatingChange,
  onDiscountChange,
  onSubChange,
  ...
  return (
    <div className="ml-10 space-y-6">
      {/* Category Filter */}
      <div>
        <div className="flex justify-between items-center cursor-pointer mb-2" onClick={() => setCategoryExpanded(!categoryExpanded)}>
        ...
        {categoryExpanded && (
          <div className="space-y-2 max-h-[220px] overflow-y-auto pr-1 visible-scrollbar">
            {categoryData.sub.map((subcat, i) => (
              <div key={i}>
                <div className="font-semibold text-gray-800 text-[15px] mb-1">{subcat.name}</div>
                {subcat.sub.map((item, j) => (
                  <label key={j} className="flex items-center gap-2 text-[14px] text-gray-700 cursor-pointer ml-2 mb-1">
                    <input
                      type="checkbox"
...
export default ElectronicsSidebarFilter;
